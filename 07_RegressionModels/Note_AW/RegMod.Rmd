...
The data frame `diamond` contains **`r nrow(diamond)`** observations on **`r ncol(diamond)`** variables.
```{r}
str(diamond)
```
...
plot(diamond$carat, diamond$price,  
     xlab = "Mass (carats)", ylab = "Price (SIN $)", bg = "lightblue", 
...
Thus $`r round(coef(fit2)[1],2)` is the expected price for the average sized diamond of the data (`r mean(diamond$carat)` carats).
...
We expect  a `r round(coef(fit)[2], 2) / 10` (SIN) dollar change in price for every 1/10th of a carat increase in mass of diamond. It is the same if we rescale the Xs and refit
```{r, echo = TRUE}
...
```{r, echo = FALSE, fig.height=6, fig.width=6, results='hide'}
...
sumCoef[1,1] + c(-1, 1) * qt(.975, df = fit$df) * sumCoef[1, 2] # beta_0
sumCoef[2,1] + c(-1, 1) * qt(.975, df = fit$df) * sumCoef[2, 2] # beta_1
```
...
```{r, fig.height=5, fig.width=10, results='hide'}
par(mfrow = c(1, 2))
